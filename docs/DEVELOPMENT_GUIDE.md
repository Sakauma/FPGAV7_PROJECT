...
首次克隆后建议先打开工程，检查 IP Status；如 Vivado 提示重新生成 output products，按提示生成即可。仓库会保留 MIG、PCIe 和 SRIO 补丁流程必需的少量 DCP、RTL 和 `.bak` 原始备份；除此之外，不要提交 `.runs`、`.cache`、`.sim`、`.hw`、日志、波形数据库和其他生成物。
