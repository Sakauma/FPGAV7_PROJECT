	...
	module	axis_rx_gather_axs2fifo#(
		...
		reg		[31:0]	dma_good_size	=	4096-512	;
	...
	//	assign	tr		=	~	axsr_info_fu	&&	~	axsr_fifo_fu	;
		...
		reg		sof_pkg	=	1	;	always@(posedge	clk)	sof_pkg	<=	rst	?	1	:	cue_pkg_done	?	1'b1	:	trv	?	0	:	sof_pkg	;
		...
		reg		[15:0]	axsr_info_di_reg	=	0	;	always@(posedge	clk)	axsr_info_di_reg	<=	axsr_info_di_byte/UB	;
	...
	//	assign	axsr_info_di	=	axsr_info_di_byte/UB	;
		...
		always@(posedge	clk)	begin
			...
			else	if(		sof_pkg			&&	~trvl	)	com_wait_en	<=	dma_wait_set	!=	0	;
		end
		
		always@(posedge	clk)	begin
			if(rst)	dma_good_size	<=	4096-512	;
			else	if(	cur_pkg_done	)	dma_good_size	<=	dma_size_set		;
			else	if(		sof_pkg			&&	~trvl	)	dma_good_size	<=	dma_size_set	;
		end
