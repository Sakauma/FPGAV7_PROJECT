	...
	//	reg		[	16-ZW	-	1:0]	cur_pkg_cunt		=	0							;
		...
		always@(posedge	clk)	begin
			...
			end	else	if	(	axsr_fifo_rd	)begin
				cur_pkg_upsz	<=	cur_pkg_upsz	-	1	;
			end	else	begin
	...
	//	assign	cur_pkg_done	=	cur_pkg_cunt	+	1	>=	cur_pkg_upsz	&&	axsr_fifo_rd		;
		assign	cur_pkg_done	=						1	>=	cur_pkg_upsz	&&	axsr_fifo_rd		;
		...
		localparam	P_RRAM_STYLE	=	QW	>	72	?	"distributed"		:	"block"	;
		localparam	P_RRAM_WR_AW	=	72	>	DW	?	clogb2(4096*8/DW)	:	5		;
		localparam	P_RRAM_RD_AW	=	72	>	QW	?	clogb2(4096*8/QW)	:	5		;
	...
	//  
	 ...
	 generate if( LOOP_NUM == 8 )		
   	ila_576X1024 ila_w32_d1024_dma (
